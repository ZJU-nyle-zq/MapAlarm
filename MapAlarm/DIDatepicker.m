...
- (void)setupViews
{
    ...
    self.selectedDateItemColor = [UIColor colorWithRed:0.910 green:0.278 blue:0.128 alpha:1.000];
}
...
- (void)setSelectedDateItemColor:(UIColor *)selectedDateItemColor
{
    _selectedDateItemColor = selectedDateItemColor;

    for (id subview in self.datesScrollView.subviews) {
        if ([subview isKindOfClass:[DIDatepickerDateView class]]) {
            DIDatepickerDateView *dateView = (DIDatepickerDateView *)subview;
            [dateView setItemSelectionColor:selectedDateItemColor];
        }
    }
}
...
- (void)updateDatesView
{
    ...
    for (NSDate *date in self.dates) {
        ...
        //[dateView setCircle:CGRectMake(currentItemXPosition, 0, kDIDatepickerItemWidth, self.frame.size.height)];
        [dateView setItemSelectionColor:self.selectedDateItemColor];
